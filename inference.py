...
def parse_args():
    ...
    parser.add_argument("--infer_list", default="./data/voc12/train.txt", type=str)
    ...
    parser.add_argument("--cam_npy", default='./cam_npy', type=str)
    parser.add_argument("--cam_png", default='./cam_png', type=str)
    parser.add_argument("--crf", default='./crf', type=str)
    parser.add_argument("--thr", default=0.22, type=float)
    ...
    if args.cam_npy is not None:
        cam_npy_folder = os.path.join(args.work_dir, args.cam_npy)
        os.makedirs(cam_npy_folder, exist_ok=True)
        args.save_type.append(cam_npy_folder)
        args.cam_npy = cam_npy_folder
    if args.cam_png is not None:
        cam_png_folder = os.path.join(args.work_dir, args.cam_png)
        os.makedirs(cam_png_folder, exist_ok=True)
        args.save_type.append(cam_png_folder)
        args.cam_png = cam_png_folder
    if args.crf:
        args.crf_list = list()
        for t in args.crf_t:
            for alpha in args.crf_alpha:
                crf_folder = os.path.join(args.work_dir, args.crf, 'crf_{}_{}'.format(t, alpha))
