...
class Net(network.resnet38d.Net):
    def __init__(self, num_classes=21):
        ...
        # projector #1 (pixel-level Semantic Contrast)
        self.proj_fc = nn.Conv2d(4096, 128, 1, bias=False)

        # projector #2 (ROI-level and Class-level Semantic Contrast)
        self.extra_F = nn.Sequential(
            nn.Conv2d(4096, 256, kernel_size=3, padding=1),
            nn.BatchNorm2d(256),
        ...
        self.from_scratch_layers = [self.fc8, self.proj_fc, self.f8_3, self.f8_4, self.f9, self.extra_F]
    ...
    def forward(self, img):
        ...
        x = d['conv6']
        f_map_1 = F.relu(self.proj_fc(x), inplace=True)
        f_map_2 = self.extra_F(x)
        ...
        return pred, cam, pred_rv, cam_rv, f_map_1, f_map_2
